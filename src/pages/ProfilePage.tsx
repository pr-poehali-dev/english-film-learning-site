...
export default function ProfilePage({ user, movies, words, onLogout }: ProfilePageProps) {
  ...
  const stats = [
    ...
    { icon: 'Film', label: 'Фильмов просмотрено', value: watchedMovies.length, color: 'text-rose-500', bg: 'bg-rose-50' },
  ...
  return (
    <div className="min-h-screen pb-20 md:pb-8 max-w-3xl mx-auto px-4 py-6">
      {/* Profile header */}
      <div className="bg-gradient-to-br from-rose-500 to-rose-700 rounded-2xl p-6 mb-6 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 mt-1">
              <Icon name="Calendar" size={12} className="text-rose-300" />
              <span className="text-rose-300 text-xs">На сайте с {new Date(user.joinedAt).toLocaleDateString('ru-RU', { month: 'long', year: 'numeric' })}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-rose-200 text-xs font-semibold uppercase tracking-wide">Streak этой недели</p>
          ...
          <div className="flex gap-1.5">
            {daysOfWeek.map((day, i) => (
              <div key={day} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="text-[10px] text-rose-300">{day}</span>
              </div>
            ))}
          </div>
        </div>
